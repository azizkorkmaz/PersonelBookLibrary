...
namespace PersonalBookLibrary.WebApi.MessageHandlers
{
            ...
            try
            {
                ...
                if (token != null)
                {
                    ...
                    User user = userService.GetByUserNameAndPassword(
                        tokenValues[0], tokenValues[1]);
                    if ( user!= null)
                    {
                        IPrincipal principal = new GenericPrincipal(
                            new GenericIdentity(tokenValues[0]), userService.GetUserRoles(user)
                            .Select(u=>u.RoleName).ToArray());
                        Thread.CurrentPrincipal = principal;//mvc için
                        HttpContext.Current.User = principal;//webapi için
                    }
